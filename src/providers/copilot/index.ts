...
async function instructions(dispatch: InvocationDispatch): Promise<string> { const path = dispatch.executor.session.instructions.localReadOnlyPath; if (!isAbsolute(path)) throw new TypeError("Copilot instructions path is not absolute"); const content = await readFile(path, "utf8"); if (`sha256:${sha256(content)}` !== dispatch.executor.session.instructions.contentIdentity) throw new TypeError("Copilot instruction identity mismatch"); return content; }
